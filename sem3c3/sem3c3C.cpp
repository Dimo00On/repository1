...
template <>
bool IsLess(const long long& first, const long long& second) {
  return first < second;
}
template <typename T, typename Q>
...
template <>
bool IsMore(const long long& first, const long long& second) {
  return first > second;
}
...
template <typename T>
double ScalarMultiply(const Point<T>& first, const Point<T>& second);
template <typename T>
double VectorMultiply(const Point<T>& first, const Point<T>& second);
...
class Point {
 public:
  ...
  double operator/(const Point& other) {
    if (IsEqual(other.x_, 0.0)) {
      if (IsEqual(x_, 0.0)) {
        if (IsEqual(other.y_, 0.0)) {
          if (IsEqual(y_, 0.0)) {
            return 1;
          }
          return kInfinity;
  ...
  bool IsCollinearTo(const Point& other) const {
    return IsEqual(VectorMultiply(*this, other), 0.0);
  }
  bool IsPerpendicularTo(const Point& other) const {
    return IsEqual(ScalarMultiply(*this, other), 0.0);
  }
  void Normalize() {
    Length();
    x_ /= length_;
...
std::ostream& operator<<(std::ostream& out, const Point<T>& point) {
  out << std::setprecision(kPrecision)
      << (IsEqual(point.GetX(), 0.0) ? 0 : point.GetX()) << " "
      << (IsEqual(point.GetY(), 0.0) ? 0 : point.GetY());
  return out;
}
template <typename T>
...
Point<double> GetPointOnLine(double a, double b,
  ...
  bool is_zero = IsEqual(b, 0.0);
  return {(is_zero ? -c / a : 0.0), (is_zero ? 0.0 : -c / b)};
}
...
class SegmentsCrossChecker {
 public:
  ...
  SegmentsCrossChecker(const Point<double>& first_begin,
      ...
      : first_begin_(first_begin),
        ...
        first_vector(first_begin, first_end),
        second_vector(second_begin, second_end),
        temp_vector(first_begin, second_begin) {
    Check();
  };
 ...
 private:
  ...
  Point<double> first_vector;
  Point<double> second_vector;
  Point<double> temp_vector;
  ...
  void Check() {
    ...
    if (temp_vector.IsCollinearTo(first_vector)) {
  ...
  void IsPointOnSegment(const Point<double>& point, const Point<double>& begin,
    ...
    temp_vector = Point<double>(begin, point);
    Point<double> vector(begin, end);
    if (temp_vector.IsCollinearTo(vector)) {
      double coef = temp_vector / vector;
      if (!IsMore(coef, 1.0) && !IsLess(coef, 0.0)) {
        answer = true;
        cross_point = point;
      }
    }
  ...
  void SegmentsCrossParallel() {
    answer = true;
    double first_coef = temp_vector / first_vector;
    if (!IsLess(first_coef, 0.0) && !IsMore(first_coef, 1.0)) {
      cross_point = second_begin_;
      return;
    }
    temp_vector = Point<double>(first_begin_, second_end_);
    double second_coef = temp_vector / first_vector;
    if (IsMore(first_coef, 1.0) && !IsMore(second_coef, 1.0)) {
      cross_point = first_end_;
      return;
    }
    if (IsLess(first_coef, 0.0) && !IsLess(second_coef, 0.0)) {
      cross_point = first_begin_;
      return;
    }
    answer = false;
  ...
  void SegmentsCrossNotParallel() {
    Line first_line(VectorType::Direction, first_vector, first_begin_);
    Line second_line(VectorType::Direction, second_vector, second_begin_);
    ...
    double first_coef = first_cross_vector / first_vector;
    double second_coef = second_cross_vector / second_vector;
    if (!IsLess(first_coef, 0.0) && !IsMore(first_coef, 1.0) &&
        !IsLess(second_coef, 0.0) && !IsMore(second_coef, 1.0)) {
      answer = true;
    }
  }
};
...
int main() {
  Point first_begin, first_end, second_begin, second_end;
